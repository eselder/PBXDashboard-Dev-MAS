...
@Component({
  selector: 'app-create-multireport',
  templateUrl: './create-multireport.component.html',
  styleUrls: ['./create-multireport.component.css']
})
export class CreateMultireportComponent implements OnInit {

  public calls: Call[];
  public extensions: Extension[];
  public reportExtensions: Extension[];
  public reportExtension: Extension;
  public extsIn: string[] = [];
  ...
  public filter_by_time: boolean = false;
  ...
  onSubmit(f: NgForm)
  {
    ...
    this.extsIn = f.value.extensions;
  ...
  {
    this.extsIn = event.target.value;
  }
  ...
  createReport(callOrigin: string = "both")
  {
    ...
    if (callOrigin == "both" && this.callScope == "both")
    {
      ...
      this.filteredCalls.forEach(call =>
        {
          if ((call.fromNumber == extIn || call.toNumber == extIn) &&   callOrigin == call.origination)
          {
            this.reportCalls.push(call);
          }
        }
        ...
        this.filteredCalls.forEach(call =>
          {
            ...
            if (isInternal && (call.toNumber == extIn || call.fromNumber == extIn ))
            {
              this.reportCalls.push(call);
            }
          }
        ...
        this.filteredCalls.forEach(call =>
          {
            ...
            if (!isExternal)
            {
              let correctNumber: boolean = (call.fromNumber == extIn || call.toNumber == extIn);
              if (correctNumber)
              {
                if (callOrigin == "outgoing" && call.from == extIn)
                {
                  this.reportCalls.push(call);
                }
                else if (callOrigin == "incoming" && call.toNumber == extIn)
                {
                  this.reportCalls.push(call);
                }
              }
